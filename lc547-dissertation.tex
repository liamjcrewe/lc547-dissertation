...
This checks to see if the token exists and is going to expire in less than an hour. If these conditions are met, it refreshed the token using a PUT request to the \enquote{/v1/tokens} endpoint of the SelfReflect API.
...
The edit email and edit password forms are relatively simple. They use the same validation rules as the register page, require the current password to be provided for security reasons, and send a PUT request to the \enquote{/v1/users/:id} endpoint of the SelfReflect API. The body of this request contains the full original user, with either the email or the newPassword field changed, for edit email and edit password respectively. See section \ref{sec:apiimplusers} for further details on this.
...
\subsubsection{Connect Existing Apps}
...
\subsection{Connect Twitter} \label{sec:apiconnecttwitter}


\subsection{Connect Strava} \label{sec:apiconnectstrava}


\subsection{Users Endpoint} \label{sec:apiimplusers}
